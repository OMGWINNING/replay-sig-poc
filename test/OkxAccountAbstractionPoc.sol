...
import {SmartAccount} from "AccountAbstraction/wallet/SmartAccount.sol";
import {SmartAccountProxyFactory} from "AccountAbstraction/wallet/SmartAccountProxyFactory.sol";
import {DefaultCallbackHandler} from "AccountAbstraction/wallet/handler/DefaultCallbackHandler.sol";
...
contract TestOkxAccountAbstractionPoc is Test {
    DefaultCallbackHandler handler = new DefaultCallbackHandler();
    SmartAccount _singleton = new SmartAccount(payable(address(0)), address(handler), "SA", "1");
    SmartAccountProxyFactory factory = new SmartAccountProxyFactory(payable(address(_singleton)), address(0));
    ERC20 erc20 = new ERC20("a", "a");
    Permit2 permit2 = new Permit2();
    SmartAccount accountA;
    SmartAccount accountB;
    ...
    function setUp() public {
        ...
        bytes memory initializer = abi.encodeWithSelector(selector, owner);

        accountA = SmartAccount(payable(factory.createAccount(address(_singleton), initializer, 0)));
        accountB = SmartAccount(payable(factory.createAccount(address(_singleton), initializer, 1)));
